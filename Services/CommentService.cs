...
namespace harmadik_backend_app.Services;

public static class CommentService
{
    static List<Comment> Comments { get; }
    static int nextId = 501;
    static CommentService()
     {
        Comments = new List<Comment> {
            new Comment  { postId = 1, Id = 1, name = "id labore ex et quam laborum", email = "Eliseo@gardner.biz", body="laudantium enim quasi est quidem magnam voluptate ipsam eos\ntempora quo necessitatibus\ndolor quam autem quasi\nreiciendis et nam sapiente accusantium" },
new Comment {
    postId = 1,
    Id = 2,
  ...
  new Comment {
    ...
    public static List<Comment> GetAll() => Comments;

    public static Comment? Get(int id) => Comments.FirstOrDefault(p => p.Id == id);
    ...
    public static void Update(Comment comment)
     {
        var index = Comments.FindIndex(p => p.Id == comment.Id);
        if(index == -1)
            return;
